...
| --------------------- | ------- | -------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `hibpBaseUrl`         | URL     | `https://api.pwnedpasswords.com` | Base URL of the HIBP range API (the policy appends `/range/{prefix}`). Override only for a HIBP-compatible mirror or proxy. Must be an absolute URL or startup fails. |
| `failOpen`            | boolean | `true`                           | If `true`, allow the password when the HIBP lookup fails. If `false`, reject it.                                                                                      |
| `lookupTimeoutMillis` | integer | `3000`                           | Total time budget for a single HIBP lookup; on expiry the request is aborted and treated as a failure (handled by `failOpen`).                                        |
| `userAgent`           | string  | plugin name + version            | `User-Agent` header sent on each HIBP lookup. Defaults to the plugin name and version; a blank value falls back to that default.                                      |
